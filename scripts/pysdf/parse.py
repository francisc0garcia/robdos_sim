...
class World(object):
    ...
    def plot(self, graph, prefix=''):
        ...
        for model in self.models:
            ...
            graph.add_edge('world',
                           model.name + '::' + model.root_link.name if prefix is not None else model.root_link.name)
    ...
    def plot(self, graph, prefix=''):
        ...
        for link in self.links:
            graph.add_node(full_prefix + link.name,
                           label=full_prefix + link.name + '\\nrel: ' + homogeneous2tq_string_rounded(
                               link.pose) + '\\nabs: ' + homogeneous2tq_string_rounded(link.pose_world))
        subgraph = graph.add_subgraph([full_prefix + link.name for link in self.links], 'cluster_' + self.name,
                                      color='gray', label=self.name + '\\nrel: ' + homogeneous2tq_string_rounded(
    ...
    def add_urdf_elements(self, node, prefix):
        ...
        if self.tree_parent_link.parent_model == self.parent_model:
            ...
            pose2origin(jointnode,
                        concatenate_matrices(inverse_matrix(parent_pose_world), self.tree_child_link.pose_world))
        if self.type == 'revolute' and self.axis.lower_limit == 0 and self.axis.upper_limit == 0:
            ...
            dummylinknode = ET.SubElement(node, 'link',
                                          {'name': sdf2tfname(full_prefix + self.name + '::revolute_dummy_link')})
            childnode.attrib['link'] = dummylinknode.attrib['name']
            dummyjointnode = ET.SubElement(node, 'joint',
                                           {'name': sdf2tfname(full_prefix + self.name + '::revolute_dummy_joint')})
            ...
            self.axis2.add_urdf_elements(dummyjointnode, concatenate_matrices(inverse_matrix(self.pose_world),
        ...
        self.axis.add_urdf_elements(jointnode,
                                    concatenate_matrices(inverse_matrix(self.pose_world), self.parent_model.pose_world))
...
class Axis(object):
    ...
    def __repr__(self):
        ...
        self.xyz, ' use_parent_model_frame=%s,' % self.use_parent_model_frame if self.version >= 1.5 else '',
        self.lower_limit, self.upper_limit, self.effort_limit, self.velocity_limit)
    ...
    def add_urdf_elements(self, node, modelCBTjoint):
        if (self.version <= 1.4) or (
                self.version >= 1.5 and self.use_parent_model_frame):  # SDF 1.4 axis is specified in model frame
        ...
        limitnode = ET.SubElement(node, 'limit', {'lower': str(self.lower_limit), 'upper': str(self.upper_limit),
                                                  'effort': str(self.effort_limit),
                                                  'velocity': str(self.velocity_limit)})
...
class Inertia(object):
    ...
    def __repr__(self):
        return 'Inertia(ixx=%s, ixy=%s, ixz=%s, iyy=%s, iyz=%s, izz=%s)' % (
...
class LinkPart(SpatialEntity):
        ...
        for gtype in self.gtypes:
            ...
            if typenode != None:
                ...
                if gtype == 'box':
                    ...
                    self.geometry_data = {'uri': get_tag(typenode, 'uri'),
                                          'scale': get_tag(typenode, 'scale', '1.0 1.0 1.0')}
    ...
    def add_urdf_elements(self, node, prefix, link_pose, part_type):
        if not self.geometry_type:
            return
        partnode = ET.SubElement(node, part_type,
                                 {'name': sdf2tfname(prefix + '::' + self.name if prefix else self.name)})
        ...
        if self.geometry_type == 'box':
            ...
            cylindernode = ET.SubElement(geometrynode, 'cylinder', {'radius': self.geometry_data['radius'],

...
from parse import SDF, sdf2tfname, models_paths
from conversions import *
from naming import *
#import scripts.pysdf
...
class Gazebo2Rviz:
    ...
    def publish_tf(self):
        ...
        for (link_idx, link_name) in enumerate(self.link_states_msg.name):
            poses[link_name] = pose_msg2homogeneous(self.link_states_msg.pose[link_idx])
        ...
        for (link_idx, link_name) in enumerate(self.link_states_msg.name):
            ...
            self.tfBroadcaster.sendTransform(translation, quaternion, rospy.get_rostime(), sdf2tfname(link_name),
                                             sdf2tfname(parentinstance_link_name))
